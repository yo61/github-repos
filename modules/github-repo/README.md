...
| <a name="input_topics"></a> [topics](#input\_topics) | The list of topics on the repository. Leave null (the default) to leave the<br/>attribute unmanaged, so a repo whose topics were set outside Terraform sees<br/>no drift. An explicit [] is NOT the same thing: the provider marks topics<br/>optional+computed, so null means "leave alone" while [] means "manage it,<br/>and it is empty" -- which removes every existing topic. | `set(string)` | `null` | no |
