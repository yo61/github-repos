  ...
  dynamic "security_and_analysis" {
    ...
    content {
      dynamic "advanced_security" {
        for_each = local.security_and_analysis.advanced_security == null ? [] : [local.security_and_analysis.advanced_security]
        content {
          status = advanced_security.value ? "enabled" : "disabled"
        }
      }
      dynamic "secret_scanning" {
        for_each = local.security_and_analysis.secret_scanning == null ? [] : [local.security_and_analysis.secret_scanning]
        content {
          status = secret_scanning.value ? "enabled" : "disabled"
        }
      }
      dynamic "secret_scanning_push_protection" {
        for_each = local.security_and_analysis.secret_scanning_push_protection == null ? [] : [local.security_and_analysis.secret_scanning_push_protection]
        content {
