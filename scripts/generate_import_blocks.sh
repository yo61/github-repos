...
#
# Remove imports-<ORG>.tf after `task apply` (the blocks are one-shot).
#
# ORG is required and names a data/<org>/ directory. It is not defaulted: a
# default silently generates blocks for the wrong org, and every address in the
# file would then be wrong in a way the plan reports as "will be created".
#
# Usage:
#   ORG=yo61 scripts/generate_import_blocks.sh              # every managed repo
#   ORG=ycst-org-uk scripts/generate_import_blocks.sh a b   # subset (each needs YAML)
#
...
if [[ -z "${ORG:-}" ]]; then
  echo "error: ORG is required, e.g. ORG=ycst-org-uk $0" >&2
  exit 1
fi
...
# Terraform module labels cannot contain hyphens, so main.tf names the module
# after the org with hyphens replaced: ycst-org-uk -> module.org_ycst_org_uk.
# The same substitution the Taskfile's `plan` target makes for -target.
MODULE_ORG="${ORG//-/_}"
...
{
  printf '# Generated by scripts/generate_import_blocks.sh\n'
  printf '# Imports every managed %s repo and its terraform resources into state.\n' "${ORG}"
  printf '# Remove this file after running task apply.\n\n'
  printf '%s' "${buffer}"
} > "${IMPORTS_FILE}"
